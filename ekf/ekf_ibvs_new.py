...
class EKF_IBVS_One_Point(object):
    ...
    def __init__(self, x0, P0, Q, R):   
        ...
        Args:
            x0 (np.array): initial state, size 8
            P0 (np.array): initial covariance, size 8x8
            Q (np.array): process noise covariance, size 8x8
        ...
        x, y, Z, d1, d2, d3, dd1, dd2 = symbols('x y Z d1 d2 d3 dd1 dd2')
        ...
        dot_d = Matrix([[dd1], [dd2], [0]])
        dot_dot_d = Matrix([[0], [0]])
        continuous_dynamics = Matrix.vstack(dot_x_y_Z, dot_d, dot_dot_d)

        # Define discrete dynamics
        discrete_dynamics = continuous_dynamics * dt + Matrix([[x], [y], [Z], [d1], [d2], [d3], [dd1], [dd2]])
        self.discrete_dynamics = lambdify([dt, x, y, Z, d1, d2, d3, dd1, dd2, Vx, Vy, Vz, Wx, Wy, Wz], discrete_dynamics, "numpy")

        # Define process partial derivatives
        discrete_process_jacobian = simplify(discrete_dynamics.jacobian(Matrix([x, y, Z, d1, d2, d3, dd1, dd2])))
        self.discrete_process_jacobian = lambdify([dt, x, y, Z, d1, d2, d3, dd1, dd2, Vx, Vy, Vz, Wx, Wy, Wz], discrete_process_jacobian, "numpy")

        # Define measurement partial derivatives
        self.measurement_jacobian = np.eye(3,8)
...
class EKF_IBVS(object):
    ...
    def __init__(self, num_points, x0, P0, Q, R, fx, fy, cx, cy):   
        ...
        Args:
            ...
            x0: initial state, size num_pointsx8
            P0: initial covariance, size 8x8
            Q: process noise covariance, size 8x8
            R: measurement noise covariance, size 3x3
            fx: focal length in x axis, float
            fy: focal length in y axis, float
            cx: principal point in x axis, float
            cy: principal point in y axis, float
        """
        ...
        if x0.shape != (num_points, 8):
            raise ValueError('x0 should have size num_pointsx8.')
    ...
    def get_updated_state(self):
        ...
        Returns:
            x: state, size num_pointsx8
        """
        x = np.zeros((len(self.ekf_list),8))
        for i, ekf in enumerate(self.ekf_list):
            x[i,:] = ekf.x
    ...
    def get_updated_covariance(self):
        ...
        Returns:
            P: covariance, size num_pointsx8x8
        """
        P = np.zeros((len(self.ekf_list), 8, 8))
        for i, ekf in enumerate(self.ekf_list):
            P[i,:,:] = ekf.P
        return P
    ...
    def get_predicted_covariance(self):
        ...
        Returns:
            P: covariance, size 8x8xnum_points
        """
        P = np.zeros((len(self.ekf_list), 8, 8))
        for i, ekf in enumerate(self.ekf_list):
            P[i,:,:] = ekf.P_pre
